...
def strategy(date, context):
    ...
    if context.indicator.SMA.loc[date, "SPYSMA200"] < context.data["Adj Close"].loc[date, "SPY"]:
        context.order_by_cash_weight("SPY", 1, context.data["Adj Close"].loc[date, "SPY"])
    else:
        context.order_by_cash_weight("SPY", 0, context.data["Adj Close"].loc[date, "SPY"])
    # else:
    #     i = 0
...
if __name__ == '__main__':
    ...
    context.load_asset_list(asset_list, "2000-01-01", ytd)
